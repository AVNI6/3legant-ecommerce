...
import { fetchAddresses, createAddressThunk, updateAddressThunk, removeAddressThunk, setDefaultAddressThunk, selectAddresses, hydrateAddresses } from "@/store/slices/addressSlice";
...
const AddressContent = ({ initialAddresses, initialTotalCount, initialPage, userId }: Props) => {
  ...
  }, [dispatch, initialAddresses, initialTotalCount, userId]);
  ...
  useEffect(() => {
    ...
    dispatch(fetchAddresses({ userId, page: currentPage, pageSize }));
  }, [userId, currentPage, initialPage, dispatch]);
  ...
  const setDefaultAddress = (addressId: string) => {
    ...
    dispatch(setDefaultAddressThunk({ userId, addressId }))
      .unwrap().catch(err => {
        toast.error(err);
        const def = addresses.find((a: any) => a.is_default);
        setDefaultAddressId(def?.id || null);
  ...
  const createAddress = async (newAddress: AddressType) => {
    ...
    setIsSaving(true);
    try {
      await dispatch(createAddressThunk({ userId, address: newAddress })).unwrap();
  ...
  const updateAddress = async (address: any, updated: AddressType) => {
    ...
    setIsSaving(true);
    try {
      await dispatch(updateAddressThunk({ addressId: address.id, updates: updated, userId })).unwrap();
