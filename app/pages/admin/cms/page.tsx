...
import { useAdminBanners, useAdminBlogs, useSaveBanner, useDeleteBanner, useSaveBlog, useDeleteBlog } from "@/hooks/admin/use-admin-queries"
...
export default function CMSPage() {
  ...
  const { data: banners = [], isLoading: isLoadingBanners, refetch: refetchBanners } = useAdminBanners()
  const { data: blogs = [], isLoading: isLoadingBlogs, refetch: refetchBlogs } = useAdminBlogs()
  
  const { mutate: saveBanner, isPending: isSavingBanner } = useSaveBanner()
  const { mutate: deleteBannerMutation, isPending: isDeletingBanner } = useDeleteBanner()
  ...
  const handleUpload = async (file: File, type: "banner" | "author" | "cover") => {
    setUploading(type)
    ...
    try {
      ...
      if (res.ok && json.url) {
        if (type === "banner") setEditBanner(prev => prev ? { ...prev, image_url: json.url } : null)
  ...
  const onSaveBanner = () => {
    if (!editBanner?.title || !editBanner?.image_url) {
      return toast("Title and image are required", "error")
    }
    const payload = { ...editBanner }
    saveBanner({ id: editBanner.id, payload }, {
      onSuccess: () => {
        toast("Banner saved")
        setEditBanner(null)
        refetchBanners()
      },
      onError: (err: any) => toast(err.message || "Failed to save banner", "error")
    })
  }
        ...
        <div className="flex items-center gap-2 bg-white p-1.5 rounded-2xl shadow-sm border border-gray-100">
          <button
            ...
            className={`flex items-center gap-2 px-5 py-2.5 rounded-xl text-xs font-black uppercase tracking-widest transition-all ${
              tab === "banners" ? "bg-gray-900 text-white shadow-lg" : "text-gray-400 hover:text-gray-900 hover:bg-gray-50"
            }`}
          >
            <HiOutlinePhotograph className="w-4 h-4" /> Banners
          </button>
          <button
            onClick={() => setTab("blogs")}
            className={`flex items-center gap-2 px-5 py-2.5 rounded-xl text-xs font-black uppercase tracking-widest transition-all ${
              tab === "blogs" ? "bg-gray-900 text-white shadow-lg" : "text-gray-400 hover:text-gray-900 hover:bg-gray-50"
            }`}
          >
            <HiOutlineDocumentText className="w-4 h-4" /> Blogs
          </button>
        </div>
      </div>
      ...
      {tab === "banners" ? (
        <div className="space-y-6">
          <div className="flex items-center justify-between">
            <h2 className="text-xs font-black text-gray-400 uppercase tracking-[0.2em]">Promotion Banners</h2>
            <button
              onClick={() => setEditBanner({ is_active: true, position: banners.length })}
            ...
            >
              <HiPlus className="w-4 h-4" /> Add New Banner
            </button>
          </div>
          ...
          {isLoadingBanners ? (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
               {[...Array(4)].map((_, i) => (
                 <div key={i} className="aspect-video bg-gray-50 rounded-3xl animate-pulse" />
               ))}
            </div>
          ) : banners.length === 0 ? (
            <div className="bg-white rounded-[40px] border border-gray-50 p-20 text-center">
               <HiOutlinePhotograph className="w-16 h-16 text-gray-100 mx-auto mb-4" />
               <p className="text-gray-400 font-bold italic">No banners configured for the storefront.</p>
            </div>
          ) : (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {banners.map(banner => (
                <div key={banner.id} className="group h-full flex flex-col bg-white rounded-3xl border border-gray-100 shadow-sm hover:shadow-xl hover:-translate-y-1 transition-all overflow-hidden relative">
                  <div className="aspect-[16/9] relative overflow-hidden bg-gray-50">
                    ...
                    <div className="absolute top-4 left-4 flex gap-2">
                       <span className={`px-2 py-1 rounded-lg text-[9px] font-black uppercase tracking-widest ${banner.is_active ? "bg-green-500 text-white" : "bg-gray-400 text-white"}`}>
                          {banner.is_active ? "Live" : "Inactive"}
                       </span>
                       {banner.category && (
                         <span className="px-2 py-1 bg-white/90 backdrop-blur rounded-lg text-[9px] font-black uppercase tracking-widest text-gray-900">
                            {banner.category}
                         </span>
                       )}
                    </div>
                  ...
                  <div className="p-6 flex-1 flex flex-col">
                    <div className="mb-4">
                      <h3 className="font-black text-gray-900 text-lg leading-tight uppercase tracking-tight line-clamp-1">{banner.title}</h3>
                      <p className="text-xs text-gray-400 font-medium mt-1 line-clamp-2">{banner.subtitle || "No subtitle provided."}</p>
                    </div>
                    
                    <div className="mt-auto pt-4 border-t border-gray-50 flex items-center justify-between">
                      <div className="flex items-center gap-1.5">
                        <span className="text-[10px] font-black text-gray-300 uppercase tracking-widest">Pos:</span>
                        <span className="text-xs font-black text-gray-900">{banner.position}</span>
                      </div>
                      <div className="flex items-center gap-1">
                        <button onClick={() => setEditBanner(banner)} className="p-2 hover:bg-gray-50 rounded-xl text-gray-400 hover:text-gray-900 transition-colors">
                          <HiOutlinePencilAlt className="w-5 h-5" />
                        </button>
                        <button onClick={() => setDeleteTarget({ type: "banners", id: banner.id })} className="p-2 hover:bg-red-50 rounded-xl text-gray-400 hover:text-red-500 transition-colors">
        ...
        <div className="space-y-6">
          <div className="flex items-center justify-between">
            <h2 className="text-xs font-black text-gray-400 uppercase tracking-[0.2em]">Editorial Content</h2>
            <button
          ...
          <div className="bg-white rounded-[40px] shadow-2xl w-full max-w-lg overflow-hidden relative border border-white/20">
            ...
            <div className="p-10">
              <h2 className="text-2xl font-black text-gray-900 mb-8 uppercase tracking-tight">
                {editBanner.id ? "Configure Banner" : "New Placement"}
              </h2>

              <div className="space-y-6 max-h-[70vh] overflow-y-auto pr-2 custom-scrollbar">
                <div className="aspect-video bg-gray-100 rounded-3xl overflow-hidden relative border-2 border-dashed border-gray-200 group">
                  {editBanner.image_url ? (
                    <img src={editBanner.image_url} alt="" className="w-full h-full object-cover" />
                  ) : (
                  ...
                  <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center">
                    <button 
                      ...
                      className="bg-white text-gray-900 px-6 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest shadow-xl"
                    >
                      {uploading === "banner" ? "UPLOADING..." : "REPLACE IMAGE"}
                    </button>
                    <input id="banner-upload" type="file" className="hidden" accept="image/*" onChange={e => e.target.files?.[0] && handleUpload(e.target.files[0], "banner")} />
                  </div>
                </div>
                ...
                <div className="space-y-4 pt-4">
                   <div className="grid grid-cols-1 gap-4">
                      <div className="space-y-1.5">
                        ...
                        <input value={editBanner.title || ""} onChange={e => setEditBanner({...editBanner, title: e.target.value})} className="w-full p-4 bg-gray-50 border-none rounded-2xl text-sm font-bold focus:ring-2 focus:ring-blue-500 outline-none transition-all placeholder:text-gray-300" placeholder="e.g. Summer Collection '24" />
                      </div>
                      <div className="space-y-1.5">
                        <label className="text-[10px] font-black uppercase tracking-widest text-gray-400 ml-1">Subtitle</label>
                        <input value={editBanner.subtitle || ""} onChange={e => setEditBanner({...editBanner, subtitle: e.target.value})} className="w-full p-4 bg-gray-50 border-none rounded-2xl text-sm font-bold focus:ring-2 focus:ring-blue-500 outline-none transition-all placeholder:text-gray-300" placeholder="e.g. Up to 40% off across all items" />
                      </div>
                   </div>

                   <div className="grid grid-cols-2 gap-4">
                      <div className="space-y-1.5">
                        <label className="text-[10px] font-black uppercase tracking-widest text-gray-400 ml-1">Internal Key</label>
                        <input value={editBanner.key || ""} onChange={e => setEditBanner({...editBanner, key: e.target.value})} className="w-full p-4 bg-gray-50 border-none rounded-2xl text-[10px] font-mono font-bold focus:ring-2 focus:ring-blue-500 outline-none transition-all" placeholder="home_main" />
                      </div>
                      <div className="space-y-1.5">
                        <label className="text-[10px] font-black uppercase tracking-widest text-gray-400 ml-1">Placement Pos.</label>
                        <input type="number" value={editBanner.position || 0} onChange={e => setEditBanner({...editBanner, position: +e.target.value})} className="w-full p-4 bg-gray-50 border-none rounded-2xl text-sm font-bold focus:ring-2 focus:ring-blue-500 outline-none transition-all" />
                      </div>
                   </div>

                   <div className="space-y-1.5">
                      <label className="text-[10px] font-black uppercase tracking-widest text-gray-400 ml-1">Destination URL</label>
                      <input value={editBanner.link_url || ""} onChange={e => setEditBanner({...editBanner, link_url: e.target.value})} className="w-full p-4 bg-gray-50 border-none rounded-2xl text-sm font-bold focus:ring-2 focus:ring-blue-500 outline-none transition-all placeholder:text-gray-300" placeholder="/pages/shop?cat=summer" />
                   </div>

                   <div className="flex items-center gap-4 pt-4">
                      <button 
                        onClick={() => setEditBanner({...editBanner, is_active: !editBanner.is_active})}
                        className={`flex-1 flex items-center justify-between p-4 rounded-2xl border-2 transition-all ${
                          editBanner.is_active ? "border-green-500 bg-green-50/50" : "border-gray-100 bg-gray-50/50"
                        }`}
                      >
                         <span className="text-[10px] font-black uppercase tracking-widest text-gray-400">Live Status</span>
                         <span className={`text-[10px] font-black uppercase tracking-widest ${editBanner.is_active ? "text-green-600" : "text-gray-400"}`}>
                            {editBanner.is_active ? "ENABLED" : "DISABLED"}
                         </span>
                      </button>
                   </div>
                </div>
              ...
              <div className="flex gap-4 mt-10">
                <button onClick={() => setEditBanner(null)} className="flex-1 py-4 border border-gray-100 rounded-2xl text-[10px] font-black uppercase tracking-widest text-gray-400 hover:bg-gray-50 transition-all">DISCARD</button>
                <button onClick={onSaveBanner} disabled={isSavingBanner} className="flex-2 py-4 bg-gray-900 text-white rounded-2xl text-[10px] font-black uppercase tracking-widest hover:bg-black transition-all shadow-xl shadow-gray-200">
                   {isSavingBanner ? "SAVING..." : "COMMIT PLACEMENT"}
                </button>
              </div>
            </div>
          </div>
...
function BlogStatusBadge({ status }: { status: "draft" | "published" }) {
  return (
    <span className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-[9px] font-black uppercase tracking-widest ${
      status === "published" ? "bg-green-50 text-green-600" : "bg-orange-50 text-orange-600"
    }`}>
...
function BlogActions({ blog, setEditBlog, setDeleteTarget }: { blog: Blog, setEditBlog: (b: Blog) => void, setDeleteTarget: (t: { type: Tab, id: number }) => void }) {
  return (
    <div className="flex items-center justify-end gap-2 lg:opacity-0 lg:group-hover:opacity-100 transition-opacity">
      ...
      <button onClick={() => setDeleteTarget({ type: "blogs", id: blog.id })} className="p-2 hover:bg-red-50 rounded-xl text-gray-400 hover:text-red-500 transition-all border border-transparent hover:border-red-100">
         <HiOutlineTrash className="w-5 h-5" />
      </button>
    </div>
  )
