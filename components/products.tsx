...
const Products = ({ products, grid = "4", variant = "grid", isLoading }: Props) => {
  ...
  useEffect(() => {
    return () => {
      Object.values(addDebounceTimers.current).forEach((timer) => clearTimeout(timer));
      addDebounceTimers.current = {};
    };
  }, []);
  ...
  useEffect(() => {
    setWishlistOptimistic((prev) => {
      ...
      Object.keys(next).forEach((key) => {
        ...
        const realValue = wishlistItems.some((item) => Number(item.variant_id) === variantId);
        if (next[variantId] === realValue) {
  ...
  const isInWishlist = useCallback(
    (variantId: number) => {
      if (Object.prototype.hasOwnProperty.call(wishlistOptimistic, variantId)) {
        return wishlistOptimistic[variantId];
      }
      return wishlistItems.some((item) => Number(item.variant_id) === variantId);
    },
    [wishlistItems, wishlistOptimistic]
  );
  ...
  const cartQtyByVariant = useMemo(() => {
    ...
    cartItems.forEach((item: any) => {
      ...
      map[variantId] = (map[variantId] ?? 0) + Number(item.quantity ?? 0);
    });
    return map;
  }, [cartItems]);
  ...
  const getStockState = useCallback(
    (product: ProductType) => {
      const variantId = Number(product.variant_id || product.id);
  ...
  const handleWishlistToggle = useCallback(
    async (e: React.MouseEvent, product: ProductType, effectivePrice: number) => {
      e.preventDefault();
      e.stopPropagation();

      if (!user) {
        requireLogin(() => undefined, user);
        return;
      }
  ...
  const runAddToCart = useCallback(
    async (product: ProductType, effectivePrice: number, productReviewStats: ProductReviewStats) => {
      const { variantId, stock, inCart, isBlocked } = getStockState(product);

      if (isBlocked) {
        toast.warning(stock <= 0 ? "Item out of stock" : `Cannot add more. Total stock limit of ${stock} reached.`);
        return;
      }

      const requested = Math.max(1, Number((product as any).quantity ?? 1));
      if (inCart + requested > stock) {
        toast.warning(`Cannot add more. Total stock limit of ${stock} reached.`);
        return;
      }
      ...
      if (addToCart.fulfilled.match(resultAction)) {
        const { limitReached, alreadyInCart } = resultAction.payload as any;
        if (limitReached) {
          toast.warning("Item out of stock");
        } else {
          toast.success(alreadyInCart ? "Quantity updated" : "Item added");
        }
      ...
      const timer = addDebounceTimers.current[variantId];
      if (timer) {
        clearTimeout(timer);
        delete addDebounceTimers.current[variantId];
      }
    },
    [dispatch, user, getStockState]
  );

  const handleAddToCart = useCallback(
    (e: React.MouseEvent, product: ProductType, effectivePrice: number, productReviewStats: ProductReviewStats) => {
      e.preventDefault();
      e.stopPropagation();

      const { variantId } = getStockState(product);

      if (pendingAdd[variantId]) {
        return;
      }

      setPendingAdd((prev) => ({ ...prev, [variantId]: true }));

      const existingTimer = addDebounceTimers.current[variantId];
      if (existingTimer) {
        clearTimeout(existingTimer);
      }

      addDebounceTimers.current[variantId] = setTimeout(async () => {
        try {
          await runAddToCart(product, effectivePrice, productReviewStats);
        } finally {
          setPendingAdd((prev) => ({ ...prev, [variantId]: false }));
        }
      }, 220);
    },
    [getStockState, pendingAdd, runAddToCart]
  );

  if (!mounted && !isLoading) return null;

  return (
    <div className={isScroll ? "flex gap-6 overflow-x-auto pb-3" : `grid gap-4 ${isLoading ? "grid-cols-2 sm:grid-cols-2 md:grid-cols-3" : gridClass}`}>
      {isLoading ? (
        Array.from({ length: isScroll ? 5 : 6 }).map((_, index) => (
          <div key={index} className={isScroll ? "w-[260px] flex-shrink-0" : ""}>
            <ProductCardSkeleton />
          </div>
        ))
        ...
        products.map((product) => {
          ...
          const cardContent = (
            <div className={`font-semibold cursor-pointer ${isScroll ? "group w-[260px] flex-shrink-0" : ""} ${grid === "four" ? "flex sm:gap-6 gap-2 flex-col md:flex-row items-start md:items-center" : ""} ${grid === "three" ? "flex gap-2 md:gap-6 flex-col md:flex-row items-start md:items-stretch" : ""} ${grid === "one" || grid === "two" ? "group" : ""}`}>
              <div className={`relative bg-[#F3F5F7] rounded-lg overflow-hidden flex items-center justify-center ${grid === "four" ? "w-full sm:w-[260px] md:w-[280px] h-[250px] sm:h-[300px] flex-shrink-0" : grid === "three" ? "w-full md:w-[200px] xl:w-[250px] flex-shrink-0 md:flex md:flex-col" : "w-full"}`}>
                ...
                {!isBlocked && grid !== "three" && grid !== "four" && (
                  <button
                    ...
                    disabled={isPending}
                    className={`w-[90%] absolute z-20 bottom-4 left-1/2 -translate-x-1/2 text-white py-2 rounded-lg opacity-0 group-hover:opacity-100 transition text-xs sm:text-sm md:text-base ${isPending ? "bg-gray-500 cursor-wait" : "bg-black hover:bg-gray-800"} shadow-lg`}
                  >
                ...
                {isBlocked && grid !== "three" && grid !== "four" && (
                  <button
                    type="button"
                    disabled
                    className="w-[90%] absolute z-20 bottom-4 left-1/2 -translate-x-1/2 text-white py-2 rounded-lg text-xs sm:text-sm md:text-base bg-gray-400 cursor-not-allowed shadow-lg"
                  >
                    Out of Stock
                  </button>
                )}
              ...
              <div className={`mt-2 ${grid === "three" || grid === "four" ? "flex-1 text-left w-full min-w-0 flex flex-col justify-between" : "leading-7"}`}>
                <div>
                  ...
                  <p className={`text-sm min-[350px]:text-base sm:text-lg line-clamp-1 md:text-base lg:text-lg font-bold mb-1 ${grid === "three" ? "md:text-sm lg:text-base" : ""}`}>
                    {product.name}
                  </p>
                  ...
                  <div className="my-2 sm:my-3 space-y-2 sm:space-y-3 pointer-events-auto">
                    ...
                    <button
                      onClick={(e) => {
                        e.preventDefault();
                        e.stopPropagation();
                        handleWishlistToggle(e, product, effectivePrice);
                      }}
          ...
          return (
            <div key={`${product.id}-${product.variant_id}`} className="block">
              <Link href={`${APP_ROUTE.product}/${product.id}?variantId=${effectiveVariantId}`}>{cardContent}</Link>
            </div>
          );
        })
      )}
