...
export default function ReviewItem({ review, onDelete, onUpdate }: any) {
    ...
    const [state, setState] = useState({ showReply: false, replyTxt: "", isSubmitting: false, isEditing: false, editTxt: review.comment, editStar: review.rating, isUpdating: false, profile: null as any });
            ...
            <div className="flex gap-4 sm:gap-6">
                ...
                <div className="flex-1 space-y-3">
                        ...
                        <div className="flex flex-col sm:flex-row items-center gap-3">
                            ...
                            {user?.id === review.user_id && <div className="flex items-center gap-3"><span className="bg-gray-100 text-[10px] px-2 py-0.5 rounded-full font-bold uppercase text-gray-500">You</span>
                                {!state.isEditing && <div className="flex items-center gap-2 border-l pl-3 ml-1 border-gray-200">
                                    ...
                                    <button onClick={() => window.confirm("Delete?") && supabase.from("reviews").delete().eq("id", review.id).then(() => onDelete?.(review.id))} className="text-gray-400 hover:text-red-500 p-1"><FaTrash size={12} /></button>
                                </div>}</div>}
                        </div>
                    ...
                    <div className="flex items-center gap-6 pt-2">
                        ...
                        <button onClick={() => setState(s => ({ ...s, showReply: !s.showReply }))} className="text-xs font-bold text-[#141718] hover:text-gray-500">Reply</button>
                    </div>
                    {state.showReply && (
